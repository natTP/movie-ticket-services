...
const expressErrorResponse =
  ({ defaultError = defaultErrorResponse, enableLog = true }) =>
  (error, req, res, next) => {
    if (!error) {
      next()
    }

    const { httpStatus, message, payload } =
    ...
    if (enableLog && httpStatus >= 400) {
      try {
        console.error({
          error: {
            ...errorResponse.error,
            payload: errorResponse.error.payload
              ? JSON.stringify(errorResponse.error.payload, null, 2)
              : undefined,
          },
        })
      } catch (e) {
